...
if (isset($_POST['scanBarcode'])) {
    ...
    if ($itemData = mysqli_fetch_array($itemQuery)) {
        ...
        // Check if this item already exists in 'keluar'
        $existingItemQuery = mysqli_query($conn, "SELECT * FROM keluar WHERE idbarang = " . $itemData['idbarang']);
        if (mysqli_num_rows($existingItemQuery) > 0) {
            // If it exists, update the quantity
            $existingItemData = mysqli_fetch_array($existingItemQuery);
            $currentQty = $existingItemData['qty'] + 1; // Increment quantity
            mysqli_query($conn, "UPDATE keluar SET qty = $currentQty WHERE idkeluar = " . $existingItemData['idkeluar']);
            // Decrease stock by 1
            mysqli_query($conn, "UPDATE stock SET stock = stock - 1 WHERE idbarang = " . $itemData['idbarang']);
        } else {
            // If it doesn't exist, insert a new record
            mysqli_query($conn, "INSERT INTO keluar (idbarang, qty, penerima, tanggal) VALUES (" . $itemData['idbarang'] . ", 1, '', NOW())");
            // Decrease stock by 1
            mysqli_query($conn, "UPDATE stock SET stock = stock - 1 WHERE idbarang = " . $itemData['idbarang']);
        }
    }
...
// Delete barang keluar and update stock
if (isset($_POST['hapusbarangkeluar'])) {
    $idb = $_POST['idb']; // ID of the item
    $qty = $_POST['qty']; // Quantity of the item being deleted
    $idk = $_POST['idk']; // ID of the keluar record

    // Debug: Log the values being used for stock update
    error_log("Deleting: ID Barang: $idb, Qty: $qty, ID Keluar: $idk");

    // Get the current stock of the item
    $getdatastock = mysqli_query($conn, "SELECT * FROM stock WHERE idbarang='$idb'");
    if ($data = mysqli_fetch_array($getdatastock)) {
        $stok = $data['stock'];

        // Calculate the new stock by adding the deleted qty back to the current stock
        $newStock = $stok + $qty;

        // Update the stock table with the new quantity
        $update = mysqli_query($conn, "UPDATE stock SET stock='$newStock' WHERE idbarang='$idb'");

        // Check if the update was successful
        if ($update) {
            // Delete the record from the 'keluar' table
